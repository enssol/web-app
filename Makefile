...
pkgdatadir = $(datadir)/your_project
pkgincludedir = $(includedir)/your_project
pkglibdir = $(libdir)/your_project
pkglibexecdir = $(libexecdir)/your_project
...
DIST_COMMON = $(srcdir)/Makefile.am $(top_srcdir)/configure \
	$(am__configure_deps) $(dist_config_DATA) $(web_app_HEADERS) \
	$(am__DIST_COMMON)
...
am__DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/config.h.in \
	$(srcdir)/config/header_sources.txt \
	$(srcdir)/config/sources.txt $(srcdir)/config/test_sources.txt \
...
ACLOCAL = ${SHELL} '/Users/adriangallo/web-app/missing' aclocal-1.17
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 1
AUTOCONF = ${SHELL} '/Users/adriangallo/web-app/missing' autoconf
AUTOHEADER = ${SHELL} '/Users/adriangallo/web-app/missing' autoheader
AUTOMAKE = ${SHELL} '/Users/adriangallo/web-app/missing' automake-1.17
AWK = awk
CC = gcc
...
LIBS = -lm 
LTLIBOBJS = 
MAKEINFO = ${SHELL} '/Users/adriangallo/web-app/missing' makeinfo
MKDIR_P = mkdir -p
OBJEXT = o
PACKAGE = your_project
PACKAGE_BUGREPORT = your_email@example.com
PACKAGE_NAME = your_project
PACKAGE_STRING = your_project 1.0
PACKAGE_TARNAME = your_project
PACKAGE_URL = 
PACKAGE_VERSION = 1.0
PATH_SEPARATOR = :
RANLIB = ranlib
...
install_sh = ${SHELL} /Users/adriangallo/web-app/install-sh
...
am--refresh: Makefile
	@:
$(srcdir)/Makefile.in:  $(srcdir)/Makefile.am $(srcdir)/config/sources.txt $(srcdir)/config/header_sources.txt $(srcdir)/config/test_sources.txt $(am__configure_deps)
	@for dep in $?; do \
...
$(srcdir)/config/sources.txt $(srcdir)/config/header_sources.txt $(srcdir)/config/test_sources.txt $(am__empty):
...
web_app$(EXEEXT): $(web_app_OBJECTS) $(web_app_DEPENDENCIES) $(EXTRA_web_app_DEPENDENCIES) 
	@rm -f web_app$(EXEEXT)
...
install-web_appHEADERS: $(web_app_HEADERS)
	@$(NORMAL_INSTALL)
	@list='$(web_app_HEADERS)'; test -n "$(web_appdir)" || list=; \
	if test -n "$$list"; then \
	  echo " $(MKDIR_P) '$(DESTDIR)$(web_appdir)'"; \
	  $(MKDIR_P) "$(DESTDIR)$(web_appdir)" || exit 1; \
	fi; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	...
	while read files; do \
	  echo " $(INSTALL_HEADER) $$files '$(DESTDIR)$(web_appdir)'"; \
	  $(INSTALL_HEADER) $$files "$(DESTDIR)$(web_appdir)" || exit $$?; \
	done
...
uninstall-web_appHEADERS:
	...
	@list='$(web_app_HEADERS)'; test -n "$(web_appdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	dir='$(DESTDIR)$(web_appdir)'; $(am__uninstall_files_from_dir)
...
recheck: all $(check_PROGRAMS)
	...
	$(MAKE) $(AM_MAKEFLAGS) $(TEST_SUITE_LOG) \
	        ...
	        TEST_LOGS="$$log_list"; \
	exit $$?
tests/test_program.log: tests/test_program
	@p='tests/test_program'; \
	b='tests/test_program'; \
...
tests/test_error_handler.c
tests/test_env_loader.c
